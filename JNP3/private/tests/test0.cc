...
void testuj_operatory_zdradliwe(const string &s = "ryboplazokon") {
    ...
    // ZakÅ‚adam, Å¼e wtedy nic siÄ™ nie dzieje (bo byÅ‚by overflow)
    // Ale chyba to nie jest jedyne sensowne zaÅ‚oÅ¼enie
    // W kaÅ¼dym razie jest przynajmniej jasnoÅ›Ä‡ co do zmniejszania o zbyt duÅ¼o
    //
    /* Czy po wykonaniu operacji -= mamy zmniejszyÄ‡ iloÅ›Ä‡ dostÄ™pÃ³w o tyle o
     * ile jest moÅ¼liwe pÃ³ki nie dojdziemy do 0 czy jeÅ›li dostajemy rozkaz
     * zmniejszenia o wiÄ™cej niÅ¼ moÅ¼emy to nic nie robimy?

    PrzejdÅº do wiadomoÅ›ci wyÅ¼ej (â€žparentâ€) | Odpowiedz
    Obraz Maciej Zielenkiewicz
    Odp: Zadanie 3
    Maciej Zielenkiewicz w dniu Thursday, 7 November 2013, 15:32 napisaÅ‚(a)


    JeÅ¼eli nie jest moÅ¼liwe zmniejszenie liczby dostÄ™pÃ³w o Å¼Ä…danÄ… wartoÅ›Ä‡ to nic nie robimy.
*/
...
void testy_z_forum() {
    ...
    ss1.str(""), ss2.str("");
//  PrzemysÅ‚aw Gumienny w dniu Tuesday, 5 November 2013, 20:48 napisaÅ‚(a)
    s1 += 0;
    s2 -= 0;
    s3 *= 1;
...
// Tomasz StÄ™czniewski w dniu Wednesday, 6 November 2013, 15:26 napisaÅ‚(a)
    {
...
> Zamieszczam przykÅ‚ad. ProszÄ™ o wskazanie prawdiÅ‚owego out'a
>*/
 ...
 assert(ss.str() == "ALARM: WLAMANIE\n");
        ...
        // Dawid Åazarczyk w dniu Sunday, 10 November 2013, 10:57 napisaÅ‚(a)
    {
    stringstream ss;
    Sejf a("A", 1), b("B", 1);
    b += 1;
...
// Hubert Tarasiuk w dniu Saturday, 16 November 2013, 21:43 napisaÅ‚(a)
  {
  ...
  {
      // Jacek KoziÅ„ski w dniu Sunday, 17 November 2013, 00:16 napisaÅ‚(a)
     string s = "kot";
     Sejf s1("pies"), s2(s);
     Sejf s3 = std::move(s1);
     s1 = std::move(s2);
